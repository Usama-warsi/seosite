...
class SEOReport implements ToolInterface
{
    ...
    public function handle(Request $request, Tool $tool)
    {
        ...
        $seotools = DB::select("SELECT * FROM `page_translations`");
        try {
            ...
            if ($tool->is_home == 1 && empty($request->route()->parameters)) {
                ...
                return view('tools.pages.seo-report', compact('results','tool', 'plans', 'faqs', 'properties', 'relevant_tools','seotools'));
            }
            return view('tools.seo-report', compact('results', 'tool','seotools'));
        } catch (\Exception $e) {
    ...
    public function index(Tool $tool, $relevant_tools)
    {
